...
### Added
- **CloudFormation ApiGatewayV2 Integration + Route provisioners** — `AWS::ApiGatewayV2::Integration` and `AWS::ApiGatewayV2::Route` are now supported as CloudFormation resource types. This completes the API Gateway V2 CloudFormation surface alongside the existing `Api` and `Stage` provisioners, enabling full CDK HTTP API deployments (`HttpApi.addRoutes()`) to ministack. Both provisioners support `Fn::GetAtt` (`IntegrationId`, `RouteId`) and idempotent delete.
